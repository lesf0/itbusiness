...
$blocks_all=array(
	...
	'separator' => array(
		'classes' => 'h-5 separator-bg bg-contain bg-center bg-no-repeat'
	),

	'form' => array(
		'classes' => 'form-bg bg-100pc-auto bg-top'
	),
...
?><!DOCTYPE html><html><head><? require INCLUDES."/header.php" ?></head><body><? require INCLUDES."/bodytop.php" ?><div class="body"><div class="wrapper"><?
foreach ($blocks as $__block_id => $__block) {
	if(!isset($__block['disabled'])){
		$__key = $__block['name'];
		if(isset($__block['helper'])){
			require('hlpr/'.$__key.'.php');
		}
		if(isset($__block[CLASSES])){
			$__classes = ' ' . $__block[CLASSES];
		}else{
			$__classes = '';
		}
		?><div class="block<?= $__classes ?>" id="block<?= $__block_id ?>"><? require TMPL.'/'.$__key.'.php' ?></div><?
	}
}
